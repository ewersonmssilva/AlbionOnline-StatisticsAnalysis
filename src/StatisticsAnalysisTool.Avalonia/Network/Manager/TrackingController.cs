...
namespace StatisticsAnalysisTool.Avalonia.Network.Manager
{
    public class TrackingController : ITrackingController
    {
        ...
        public bool IsTrackingAllowedByMainCharacter()
        {
            ...
            if (localEntity?.Value?.Name == null || string.IsNullOrEmpty(SettingsController.CurrentUserSettings.MainTrackingCharacterName))
            {
                return true;
            }

            if (localEntity.Value.Value.Name == SettingsController.CurrentUserSettings.MainTrackingCharacterName)
            {
                return true;
            }

...
using ValueType = StatisticsAnalysisTool.Avalonia.Enumerations.ValueType;
...
namespace StatisticsAnalysisTool.Avalonia.Network.Manager
{
    public class DungeonController
    {
        ...
        {
            _mainWindowViewModel.DungeonBindings.DungeonCloseTimer.IsVisible = mapType == MapType.RandomDungeon;
        }
        ...
        public void ResetDungeons()
        {
            ...
            Dispatcher.UIThread.InvokeAsync(() => { _mainWindowViewModel?.DungeonBindings?.TrackingDungeons?.Clear(); });
        }
        ...
        {
            _dungeons = await FileController.LoadAsync<List<DungeonObject>>($"{AppDomain.CurrentDomain.BaseDirectory}{ApplicationSettings.DungeonRunsFileName}");
        }

        public async Task SaveInFileAsync()
        {
            var toSaveDungeons = _dungeons.Where(x => x is { Status: DungeonStatus.Done }).ToList();
            await FileController.SaveAsync(toSaveDungeons, $"{AppDomain.CurrentDomain.BaseDirectory}{ApplicationSettings.DungeonRunsFileName}");
        }

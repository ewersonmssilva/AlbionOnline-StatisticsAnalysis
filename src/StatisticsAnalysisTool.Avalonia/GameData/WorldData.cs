...
namespace StatisticsAnalysisTool.Avalonia.GameData
{
    public static class WorldData
    {
        ...
        public static string? GetUniqueNameOrDefault(string? index)
        {
            ...
            if (name != null && splitName is { Length: > 0 } && name.ToLower().Contains('@'))
            {
        ...
        {
            var localFilePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "GameFiles", ApplicationSettings.WorldDataFileName);
        ...
        private static ObservableCollection<WorldJsonObject>? GetWorldDataFromLocal()
        {
            try
            {
                ...
                var localItemString = File.ReadAllText(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, ApplicationSettings.GameFilesDirectoryName, ApplicationSettings.WorldDataFileName), Encoding.UTF8);

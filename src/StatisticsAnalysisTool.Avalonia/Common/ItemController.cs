...
using Mount = StatisticsAnalysisTool.Avalonia.Models.ItemsJsonModel.Mount;
...
public static class ItemController
{
    ...
    {
        return Items?.FirstOrDefault(i => i.Index == index) ?? new Item();
    }

    public static Item GetItemByUniqueName(string uniqueName)
    {
        return Items?.FirstOrDefault(i => i.UniqueName == uniqueName) ?? Items?.FirstOrDefault(i => GetCleanUniqueName(i.UniqueName) == uniqueName) ?? new Item();
    }
    ...
    {
        var currentSettingsItemsJsonSourceUrl = SettingsController.CurrentUserSettings.ItemListSourceUrl;
        var url = GetSourceUrlOrDefault(ApplicationSettings.DefaultItemListSourceUrl, currentSettingsItemsJsonSourceUrl, ref currentSettingsItemsJsonSourceUrl);
        SettingsController.CurrentUserSettings.ItemListSourceUrl = currentSettingsItemsJsonSourceUrl;

        var localFilePath = $"{AppDomain.CurrentDomain.BaseDirectory}{ApplicationSettings.ItemListFileName}";
        ...
        if (File.Exists(localFilePath))
        {
            ...
            if (fileDateTime.AddDays(SettingsController.CurrentUserSettings.UpdateItemListByDays) < DateTime.Now)
            {
    ...
    private static async Task<ObservableCollection<Item>> GetItemListFromLocal()
    {
        try
        {
            ...
            var localItemString = await File.ReadAllTextAsync($"{AppDomain.CurrentDomain.BaseDirectory}{ApplicationSettings.ItemListFileName}", Encoding.UTF8);
        ...
        {
            DeleteLocalFile($"{AppDomain.CurrentDomain.BaseDirectory}{ApplicationSettings.ItemListFileName}");
    ...
    private static async Task<bool> GetItemListFromWebAsync(string url)
    {
        ...
        try
        {
            ...
            await File.WriteAllTextAsync($"{AppDomain.CurrentDomain.BaseDirectory}{ApplicationSettings.ItemListFileName}", fileString, Encoding.UTF8);
            return true;
        }
        catch
        {
    ...
    {
        var localFilePath = $"{AppDomain.CurrentDomain.BaseDirectory}{ApplicationSettings.FavoriteItemsFileName}";
        if (File.Exists(localFilePath))
        {
            try
            {
    ...
    {
        return GetItemByUniqueName(uniqueName ?? string.Empty).FullItemInformation switch
        {
    ...
    {
        var currentSettingsItemsJsonSourceUrl = SettingsController.CurrentUserSettings.ItemsJsonSourceUrl;
        var url = GetSourceUrlOrDefault(ApplicationSettings.DefaultItemsJsonSourceUrl, currentSettingsItemsJsonSourceUrl, ref currentSettingsItemsJsonSourceUrl);
        SettingsController.CurrentUserSettings.ItemsJsonSourceUrl = currentSettingsItemsJsonSourceUrl;

        var localFilePath = $"{AppDomain.CurrentDomain.BaseDirectory}{ApplicationSettings.ItemsJsonFileName}";
        ...
        if (File.Exists(localFilePath))
        {
            ...
            if (fileDateTime.AddDays(SettingsController.CurrentUserSettings.UpdateItemsJsonByDays) < DateTime.Now)
            {
    ...
    private static async Task<bool> GetItemsJsonFromWebAsync(string url)
    {
        ...
        try
        {
            ...
            var fileString = await content.ReadAsStringAsync();
            await File.WriteAllTextAsync($"{AppDomain.CurrentDomain.BaseDirectory}{ApplicationSettings.ItemsJsonFileName}", fileString, Encoding.UTF8);
            return true;
        }
        catch
        {
